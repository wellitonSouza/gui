...
function AlarmListHeader(props) {
    return (
        <div id={'alarm-list-header'} className={'alarm-header'}>
            <div>Showing&nbsp; <span>{props.quantity}</span> &nbsp;Alarms</div>
        </div>
    )
}
...
class AlarmsHistory extends Component {
    ...
    filterResults() {
        ...
        let result = this.props.historyAlarms.filter(f => severityList.includes(f.severity.toLowerCase()));
