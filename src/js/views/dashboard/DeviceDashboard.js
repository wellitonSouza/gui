...
const DeviceDashboardStore = require('../../stores/DeviceDashboardStore');
const DeviceDashboardActions = require('../../actions/DeviceDashboardActions');
...
class TemplateItem extends Component {
    ...
    render() {
        ...
        if (used_by == undefined) used_by = '0';
...
class ElementList extends Component {
    ...
    listItems(type) {
        if (this.props.list.length > 0) {
            if (type == 'devices') {
                return (
                    <div className="col s12 no-padding">
                        {
                            this.props.list.map(it => <DeviceItem key={it.id} data={it} />)
                        }
                    </div>
                );
            }
            ...
            return (
                <div className="col s12 no-padding">
                    {
                        this.props.list.map(it => <TemplateItem key={it.id} data={it} />)
                    }
                </div>
            );
        }
    ...
    render() {
        ...
        let tam = 0;
        if (this.props.list) tam = this.props.list.length;
...
class LeftPainel extends Component {
    ...
    render() {
        let main_stats = [];
        let side_stats = [];
        if (this.props.stats.mainStats) main_stats = this.props.stats.mainStats;
        if (this.props.stats.sideStats) side_stats = this.props.stats.sideStats;
        ...
        return (
            <div className="left-painel-info col s12 m12 l4 xl3">
                <div className="main-header">
                    ...
                    <label className="col s12 title">
                        {' '}
                        {this.props.mainTitle}
                        {' '}
                    </label>
                    <label className="col s12 subtitle">
                        {' '}
                        {this.props.subtitle}
                        {' '}
                    </label>
