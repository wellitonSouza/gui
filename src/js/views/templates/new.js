...
class FStore {
    ...
    removeAttr(attribute) {
        if (attribute.value != undefined && attribute.value.length > 0) {
            this.device.static_attrs = this.device.static_attrs.filter(i => i.object_id !== attribute.object_id);
        } else {
            this.device.attrs = this.device.attrs.filter(i => i.object_id !== attribute.object_id);
        }
    }
}
const TemplateFormStore = alt.createStore(FStore, 'TemplateFormStore');
...
class CreateTemplateActions extends Component {
    ...
    save(e) {
        ...
        if (ongoingOps == false) {
            const teste = JSON.parse(JSON.stringify(TemplateFormStore.getState().device));
            delete teste.id;
                            ...
                            <div className="row">
                                ...
                                <div className="input-field col s8">
                                    ...
                                    <input
                                        ...
                                        type="text"
                                        name="value"
                                        value={this.props.newAttr.value}
                                        key="protocol"
                                        onChange={this.handleChange}
                                    />
                                </div>
                ...
                <div className="list row">
                    <div className="col s10 offset-s1">
                        { ((this.props.device.attrs.length > 0) || (this.props.device.static_attrs.length > 0)) ? (
                            <span>
                                {this.props.device.attrs.map(attr => <AttrCard key={attr.object_id} {...attr} />)}
                                {this.props.device.static_attrs.map(attr => <AttrCard key={attr.object_id} {...attr} />)}
                            </span>
                        ) : (
